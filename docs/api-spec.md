...
- **POST** `/purchase-records`
  ...
  ```
- 返回：完整草稿记录（`status = draft`），响应体包含服务器生成的 `code`
- 备注：需携带登录态（JWT）或在测试环境通过 `x-user-id` 头部传入当前用户，后端基于登录用户填充 `createdBy`
- `code` 由服务器按 `PR-YYYYMMDD-XXXXXX` 自动生成并保证唯一；若未指定 `totalPrice` 将自动计算
...
- **POST** `/purchase-records/:id/confirm`（返回 HTTP 200）
  ...
  ```
- 说明：如未显式提供 `confirmedById`，默认使用当前登录用户；在事务中更新库存，如无库存记录则新建后调整；生成采购类型的库存调整记录
...
- **POST** `/usage-records`
  ...
  ```
- **说明**：需携带登录态或 `x-user-id` 头部，后端根据登录用户设置 `createdBy`
- **PATCH** `/usage-records/:id`
- **DELETE** `/usage-records/:id`
- **POST** `/usage-records/:id/confirm`（返回 HTTP 200），若未指定 `confirmedById` 将默认使用当前登录用户
...
- **POST** `/shopping-lists`
  ...
  ```
- **PATCH** `/shopping-l-lists/:id`：更新名称/备注/明细
...
> **说明**：系统会根据低库存/临期提醒自动生成 `autoGenerated=true` 的清单。这类清单名称和条目可在前端展示时提示为“来自提醒”，用户仍可在同一接口中标记条目已购或删除。自动生成逻辑依赖 MQ 事件 `shopping-lists.auto-create`。
